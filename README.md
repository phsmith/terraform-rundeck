...
│   ├── rundeck-get-job-id.gif
│   ├── rundeck-job-auto-fill.png
...
To create a new project you can simple copy files from `example` or `public` projects and modify as needed.
...
- **jobs:** Here lives the Rundeck Jobs definitions as Terraform `tfvars` files. The `tfvars` must be organized under folders `common`, `staging` and/or `prod`, that way it's possible to create the jobs for specific environments only or for general usage.
...
### Adding customized Project

By default terraform will create a project in Rundeck with the same name you add to the projects folder. However you can modify this behavior by adding the `project.properties.tfvars` file inside the folder with the project name.

**Customized Project definition example:**

```hcl
project_name = "..."

project_extra_configs = {
...
| job_schedule (**optional**) | | Schedule job with [Cron Expressions](http://www.quartz-scheduler.org/documentation/quartz-2.2.2/tutorials/tutorial-lesson-06.html) format. The job_schedule is `DISABLED` automatically on Rundeck Non-Prod instance |
...
| job_global_log_filter (**optional**) | **Default:** [] | Job global log filter expression. Know more about the options in [variables.tf](./variables.tf). See [Rundeck Log Filters Doc](https://docs.rundeck.com/docs/manual/log-filters/). Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |
...
| job_workflow_inline_script (**optional**) | **Default:** [] | Job workflow where the code is declared direct within the job definition. Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |
| job_workflow_script_file (**optional**) | **Default:** [] | Job workflow where the code is defined in a script file. Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |
| job_workflow_ansible_inline (**optional**) | **Default:** [] | Job workflow where the Ansible Playbook code is declared direct within the job definition. Know more about the options in [variables.tf](./variables.tf). Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |
...
| job_notifications_email (**optional**) | **Default:** [] | Job notifications configuration using the email type. Know more about the options in [variables.tf](./variables.tf) Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |
| job_notifications_webhook (**optional**) | **Default:** [] | Job notifications configuration using the webhooks type. Know more about the options in [variables.tf](./variables.tf) Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |
| job_notifications_plugin (**optional**) | **Default:** [] | Job notifications configuration using the plugin type. Know more about the options in [variables.tf](./variables.tf) Example: [projects/example/jobs/common/test.tfvars](projects/example/jobs/common/test.tfvars) |

## Create Jobs using YAML definition

By default terraform will create a job in Rundeck based on the variables defined above. But there is a possibility if you want to create projects using a project definition file in YAML format. This is a good option if you need more advanced recourses in your job.

To create a job using YAML definition you need to define a Terraform `.tfvars` file and a `.yml` in the folder `projects/project_name/jobs/[environment]/job-name`.

Below is a list of possible variables to define in the job `.tfvars` file for this cases.

| Variable | Options | Description |
| -------- | ------- | ----------- |
| job_load_from_yaml | `true` or `false`. **Default:** false | Force Terraform create jobs using YAML files |
| job_folder | **Default:** "" | The name of folder that Terraform will load the YAML files |
...
- First, the user make his changes in a **feat/fix/docs…etc** branch and push that branch to terraform-rundeck repo
...
```sh
# Run a Job
curl -L -X POST 'https://rundeck-staging.org/api/40/job/9072945e-867c-4589-9344-a5ac0c38d7bb/run'  \
...
curl -L -X GET 'https://rundeck-staging.org/api/40/execution/EXECUTION_ID/output?compacted=true' \
-H 'Accept: application/json' \
-H 'Content-Type: application/json' \
-H 'X-Rundeck-Auth-Token: RUNDECK_USER_TOKEN'
```
